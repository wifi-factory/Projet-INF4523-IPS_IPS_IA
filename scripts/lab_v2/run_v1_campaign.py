...
def build_expanded_scenarios() -> list[str]:
    scenarios: list[str] = []
    for scenario_id, count in V1_PLAN:
        scenarios.extend([scenario_id] * count)
    return scenarios


def main() -> int:
    parser = argparse.ArgumentParser(description="Run the lab_v2 V1 campaign plan.")
    parser.add_argument("--campaign-id", default=f"v1_campaign_{dt.datetime.now().strftime('%Y%m%d_%H%M%S')}")
